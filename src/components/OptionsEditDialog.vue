<template>
  <el-dialog title=" " :visible.sync="dialog" width="80%" @close=" cancelEdit (formModel) ">
    <div slot="title" class="header-title">
      設定 下拉選項
      <span style="padding: 3px 0">  {{ formModel.OptionGroup }}  </span>
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click=" confirmEdit (formModel) ">Update</el-button>
    </div>
  </el-dialog>
</template>
  ...
  methods: {
    ...
    async confirmEdit(row) {
      ...
      await createOrUpdateItem(row)
        ...
        .catch(error => {
          console.error(error)
          console.log('createOrUpdateItem error', error)
        })
    }
  }
}
