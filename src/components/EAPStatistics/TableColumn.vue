    ...
    >
      ...
      >
        ...
        <template slot-scope="scope">
          ...
          <div v-if="field.ID !== -1" :class="formatFontColorFromSource(field,scope.row[field.FieldID])" style="font-size:14px;">
            {{ scope.row[field.FieldID]===undefined?'': scope.row[field.FieldID].Value }}
          </div>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
        ...
        this.tabledata.map(row => {
          ...
          row.Items.map(element => {
            if (element.FieldID) {
              ...
              // }
              newrow[element.FieldID] = element
            }
          })
     ...
     * @param {*} source
     */
    formatFontColorFromSource(field, source) {
      if (field.IsLimit === true) {
        const badLower = parseFloat(field.Average) - parseFloat(field.StandardDifference * field.BadSdMultiple)
        const badUpper = parseFloat(field.Average) + parseFloat(field.StandardDifference * field.BadSdMultiple)

        const seriousBadLower = parseFloat(field.Average) - parseFloat(field.StandardDifference * field.SeriouslyBadSdMultiple)
        const seriousBadUpper = parseFloat(field.Average) + parseFloat(field.StandardDifference * field.SeriouslyBadSdMultiple)
     ...
     * @param {*} source
     */
    headerLabelFormat(source) {
      if (source.FieldName === null || source.FieldName === '') {
        return `${source.FieldID}`
      } else {
        if (source.Unit === null || source.Unit === '') {
          return `${source.FieldName}`
        } else {
          return `${source.FieldName}(${source.Unit})`
        }
      }
    }
  }
